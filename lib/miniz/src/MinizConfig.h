/* AALU only needs miniz's low-level streaming inflate (tinfl). The
...
// data. Rename so the linker can never capture them. The prefix is aalu_
// (not crosspoint_, matching this fork's identity) so this copy can never
// collide with another CrossPoint-derived fork's identically-renamed copy
// if both ever end up in the same link (e.g. via a shared vendored library).
#define tinfl_decompress aalu_tinfl_decompress
#define tinfl_decompress_mem_to_heap aalu_tinfl_decompress_mem_to_heap
#define tinfl_decompress_mem_to_mem aalu_tinfl_decompress_mem_to_mem
#define tinfl_decompress_mem_to_callback aalu_tinfl_decompress_mem_to_callback
#define mz_crc32 aalu_mz_crc32
#define mz_adler32 aalu_mz_adler32
#define mz_free aalu_mz_free
