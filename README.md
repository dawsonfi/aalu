...
- **Content-based reading position.** EPUB reading position now tracks a stable offset into the chapter's visible text, not just a page number. Resuming a book, or changing font size/margin/line spacing mid-chapter, lands you back on the same text instead of an approximate page from the old layout. Falls back gracefully for a `progress.bin` saved before this change.
