...
TEST(CredentialIntegrity, RejectsLengthMismatch) {
  const std::string data = "hunter2";
  const uint32_t crc = credential_integrity::crc32(data);
  EXPECT_FALSE(credential_integrity::validate(data, data.size() + 1, crc));
}
